...
description: Use when creating, scripting, inspecting, or debugging Isaac Sim OmniGraph or Action Graph workflows, including robot controllers, ROS 2 bridge graphs, sensor publishing graphs, simulation tick graphs, custom Python/C++ OmniGraph nodes, graph connections, node attributes, or graph execution failures.
---
...
4. Enable required extensions before creating extension-specific nodes.
5. Create or locate the graph under a stable path such as `/World/ActionGraph`.
6. Add nodes, set attributes, connect execution and data pins, then save a graph report with node paths and attributes.
7. Run the simulation or graph trigger and verify the downstream artifact: robot motion, ROS topic, sensor output, or log message.

## Read Order
- Read `references/graph-scripting.md` for Python graph construction and inspection.
- Read `references/ros2-and-sensor-graphs.md` for ROS 2 bridge, camera, Lidar, TF, odometry, and QoS graphs.
...
- Do not leave generated graphs undocumented; report graph path, nodes, attributes, connections, and required extensions.
