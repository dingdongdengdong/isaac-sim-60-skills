...
def prim_exists(prim_path: str) -> bool | None:
    try:
        import omni.usd
...
async def switch_camera_capture(args: argparse.Namespace) -> dict:
    ...
    before_camera = str(get_active_viewport_camera_path() or getattr(viewport, "camera_path", ""))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Switch the active viewport camera and capture a screenshot.")
    parser.add_argument("--camera", required=True, help="camera prim/path for the active viewport")
    parser.add_argument("--out-dir", default="outputs/viewport_debug", help="diagnostics output directory")
    parser.add_argument("--image", default="camera_capture.png", help="image path or name")
    parser.add_argument("--report", default="camera_capture_report.json", help="report JSON filename")
    parser.add_argument("--resolution", type=parse_resolution, default=(1280, 720), help="viewport resolution, e.g. 1280x720")
    parser.add_argument("--frames", type=int, default=8, help="viewport frames to wait after camera switch")
...
def main() -> None:
    ...
    report = asyncio.get_event_loop().run_until_complete(switch_camera_capture(args))
