...
description: Use when writing, reviewing, or debugging Isaac Sim Python scripts, standalone SimulationApp workflows, interactive Script Editor snippets, headless automation, extension enabling, app lifecycle, stage setup snippets, robot simulation snippets, or Isaac Sim Python environment issues.
---
...
## Workflow
1. Identify the script surface: standalone command, interactive Script Editor, Jupyter, extension code, or remote Python server.
2. If Isaac Sim is not already running, coordinate launch/runtime checks with `$isaac-sim-60-runtime`.
3. Read `references/python-entrypoints.md` before choosing `python.sh`, pip package execution, Script Editor, or notebook.
...
## Read Order
- Read `references/python-entrypoints.md` for standalone, interactive, notebook, and container choices.
- Read `references/simulation-app-patterns.md` for import order, headless mode, update loops, and shutdown.
- Read `references/extensions-and-environment.md` for extension enabling, Python paths, VS Code settings, and package installation.
